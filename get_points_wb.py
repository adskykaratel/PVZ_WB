...
def get_points(payload: list, domen: str):
    ...
    for d in data.get('value', []):
        point_data = data['value'].get(d, {})
        
        ## Проверяем наличие ключей
        address = point_data.get('address', 'Нет данных')
        wayinfo = point_data.get('wayInfo', 'Нет данных')

        ## Заполняем данные
        data_points_list.append({
            'id': int(d),
            'address': address,
            'wayInfo': wayinfo.replace('\n', ' ') if isinstance(wayinfo, str) else 'Нет данных'
        })
